...
import { resolveNuvio } from "@/lib/nuvio";

/* Nuvio Hindi lane (Server 15): XDMovies + HindMoviez via one call.
 * TV id shape matches the other lanes: /series/{tmdb}:{season}:{episode}
 * ?title=. Returns the same JSON contract HindiSources expects
...
export async function GET(
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : "nuvio failed";
    return NextResponse.json(
      { title: "", streams: [], captions: [], laneError: msg.slice(0, 160), diag: `nuvio: ${msg}` },
