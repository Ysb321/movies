...
import { resolveMoviesMod } from "@/lib/moviesmod";

/* MoviesMod (Server 13) - Hindi-dubbed DDL lane. TV id shape matches the
 * castle lane: /series/{tmdb}:{season}:{episode}?title=. Returns the
...
export async function GET(
  ...
  try {
    ...
    const out = await resolveMoviesMod({
      title,
      year: year || undefined,
      kind,
    ...
    const msg = err instanceof Error ? err.message : "moviesmod failed";
    return NextResponse.json(
      { title: "", streams: [], captions: [], laneError: msg.slice(0, 160), diag: `moviesmod: ${msg}` },
