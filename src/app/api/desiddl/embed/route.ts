...
/* Server 11 (DesiDDL) hub embed proxy - the tap-to-play path. Server-side
 * cracking proved unreliable live (bot-walls move), so the flow is now:
 * row tap -> hub page proxied into a same-origin sandboxed iframe -> the
 * USER clicks the hub's own Download / FSL / Generate buttons (a real
 * browser sails through) -> an injected capture script (link clicks,
 * submits, window.open, fresh file nodes - armed by user clicks) posts
 * the file URL to the watch page -> it auto-plays in the site player. A
 * parent poll on the iframe location backstops form/JS hops.
 * The proxy also walks hub page-chains server-side (meta refresh, vcloud
 * double-atob var url, hubcloud var url, /video/ links, /drive/ search
 * results, hx-redirect) so page-1-only entries land on their button page,
 * and short-circuits to a file shell when the entry already IS a file
 * (G-Direct dl.php?link=, Instant-style CDN hrefs). If our fetch is
 * walled, the client falls back to the raw hub page + paste box.
...
/* runs inside the proxied hub page: reports user-generated file URLs */
const CAPTURE = `(function(){if(window.__YF_CAP)return;window.__YF_CAP=1;var armedUntil=0;
function abs(u){try{return new URL(u,document.baseURI).href}catch(e){return ""}}
function fileFromUrl(u){if(!u)return "";try{var x=new URL(u,document.baseURI);
if(x.protocol!=="http:"&&x.protocol!=="https:")return "";
var link=x.searchParams.get("link");if(link&&/^https?:\\/\\//i.test(link))return link;
if(/\\.(mp4|mkv|avi|mov|webm|flv|wmv|m3u8|mpd|ts|m4v)(\\?|#|$)/i.test(x.pathname+x.search))return x.href;
if(/googleusercontent\\.com|busycdn\\.xyz/i.test(x.hostname))return x.href;
if(/pixeldrain/i.test(x.hostname)&&/\\/api\\/file\\//i.test(x.pathname))return x.href;
return ""}catch(e){return ""}}
function send(f){try{parent.postMessage({type:"yetflix-file",url:f},"*")}catch(e){}}
function arm(){armedUntil=Date.now()+6000}
function scanSend(){try{var els=document.querySelectorAll("a[href],video[src],video source[src]");
for(var i=0;i<els.length;i++){var u=els[i].getAttribute("href")||els[i].getAttribute("src")||"";
var f=fileFromUrl(abs(u));if(f){send(f);return true}}}catch(e){}return false}
document.addEventListener("click",function(ev){arm();try{
var t=ev.target&&ev.target.closest?ev.target.closest("a[href]"):null;
if(t){var f=fileFromUrl(abs(t.getAttribute("href")));if(f)send(f)}}catch(e){}},true);
document.addEventListener("submit",function(ev){arm();try{
var f=fileFromUrl(ev.target&&ev.target.action?ev.target.action:"");if(f)send(f)}catch(e){}},true);
try{window.open=function(u){arm();var f=fileFromUrl(abs(u||""));if(f)send(f);return null}}catch(e){}
try{var mo=new MutationObserver(function(){if(Date.now()<armedUntil)scanSend()});
...
/* hub-page links stay in the proxy; file links stay direct (captured) */
function rewriteAttrUrl(raw: string, pageUrl: string): string | null {
  const v = raw.trim();
...
function rewritePage(html: string, pageUrl: string): string {
  ...
  out = out.replace(/<(a|form|iframe)\b([^>]*?)>/gi, (_m, tag, attrs) => {
    const fixed = (attrs as string)
      .replace(/(href|src|action)\s*=\s*"([^"]*)"/gi, (_a, k, v) => {
        const n = rewriteAttrUrl(v, pageUrl);
        return n ? `${k}="${n}"` : _a;
      })
      .replace(/(href|src|action)\s*=\s*'([^']*)'/gi, (_a, k, v) => {
        const n = rewriteAttrUrl(v, pageUrl);
  ...
  const metaRe = /<meta\b[^>]*http-equiv\s*=\s*["']?refresh["']?[^>]*>/gi;
  out = out.replace(metaRe, (m) => {
    const cm = /content\s*=\s*["']?\s*\d+\s*;\s*url\s*=\s*([^"'>\s]+)/i.exec(m);
    if (!cm) return m;
    const n = rewriteAttrUrl(cm[1], pageUrl);
  ...
  const script = `<script>${CAPTURE}</script>`;
...
async function run(entry: string, init?: RequestInit): Promise<Response> {
  ...
  for (let hop = 0; hop < 4; hop++) {
    ...
    if (pageFile(html, final)) return htmlRes(rewritePage(html, final));
    const nx = continuation(html, final);
    if (nx && okNext(nx, entryHost) && !seen.has(nx)) {
      cur = nx;
      continue;
    }
    return htmlRes(rewritePage(html, final));
  }
